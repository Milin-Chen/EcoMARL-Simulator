...
class _ConfigCompat:
    def __init__(self):
        ...
        self.DEFAULT_RADIUS = env.DEFAULT_RADIUS
        self.MAX_ENTITIES = env.MAX_ENTITIES
        self.BREED_CD_PREY = env.BREED_CD_PREY
        self.BREED_CD_HUNTER = env.BREED_CD_HUNTER
        self.SPAWN_GROW_RATE = env.SPAWN_GROW_RATE
        self.SPAWN_MIN_SCALE = env.SPAWN_MIN_SCALE
        self.PI = env.PI
        self.TAU = env.TAU

        # 能量配置
        self.ENERGY_BASE_METABOLISM_PREY = env.ENERGY_BASE_METABOLISM_PREY
        self.ENERGY_PRODUCTION_PREY = env.ENERGY_PRODUCTION_PREY
        self.ENERGY_MOVEMENT_COST = env.ENERGY_MOVEMENT_COST
        self.ENERGY_TURNING_COST = env.ENERGY_TURNING_COST
        self.ENERGY_SPEED_REFERENCE = env.ENERGY_SPEED_REFERENCE

        # 智能体配置
        self.DEFAULT_FOV_DEG_HUNTER = agent.HUNTER_FOV_DEG
        self.DEFAULT_FOV_RANGE_HUNTER = agent.HUNTER_FOV_RANGE
        self.DEFAULT_FOV_DEG_PREY = agent.PREY_FOV_DEG
        self.DEFAULT_FOV_RANGE_PREY = agent.PREY_FOV_RANGE
        self.HUNTER_FOV_DEG = agent.HUNTER_FOV_DEG
        self.HUNTER_FOV_RANGE = agent.HUNTER_FOV_RANGE
        self.PREY_FOV_DEG = agent.PREY_FOV_DEG
        self.PREY_FOV_RANGE = agent.PREY_FOV_RANGE
        self.DEFAULT_RAY_COUNT = agent.DEFAULT_RAY_COUNT
        self.USE_ENTITY_FOV = agent.USE_ENTITY_FOV

        # 渲染配置
        self.BG_COLOR = render.BG_COLOR
        self.HUNTER_COLOR = render.HUNTER_COLOR
        self.PREY_COLOR = render.PREY_COLOR
        self.EYE_WHITE = render.EYE_WHITE
        self.EYE_PUPIL = render.EYE_PUPIL
        self.FOV_COLOR = render.FOV_COLOR
        self.RAY_COLOR = render.RAY_COLOR
        self.DEBUG_PANEL_BG = render.DEBUG_PANEL_BG
        self.DEBUG_PANEL_TEXT = render.DEBUG_PANEL_TEXT
        self.ENERGY_BAR_BG = render.ENERGY_BAR_BG
        self.ENERGY_BAR_FILL = render.ENERGY_BAR_FILL
        self.SENSOR_SAME = render.SENSOR_SAME
        self.SENSOR_OTHER = render.SENSOR_OTHER
        self.SENSOR_EMPTY = render.SENSOR_EMPTY
        self.FEED_FLASH = render.FEED_FLASH
        self.FEED_FLASH_ALPHA = render.FEED_FLASH_ALPHA
        self.FEED_ENERGY_DELTA_THRESHOLD = render.FEED_ENERGY_DELTA_THRESHOLD
        self.FEED_PULSE_GAIN = render.FEED_PULSE_GAIN
        self.FEED_PULSE_DECAY = render.FEED_PULSE_DECAY
        self.FEED_PULSE_MAX = render.FEED_PULSE_MAX
        self.FEED_RING_MIN_SCALE = render.FEED_RING_MIN_SCALE
        self.FEED_RING_MAX_SCALE = render.FEED_RING_MAX_SCALE
        self.FEED_RING_THICKNESS = render.FEED_RING_THICKNESS
        self.SWALLOW_COLOR = render.SWALLOW_COLOR
        self.SWALLOW_ALPHA = render.SWALLOW_ALPHA
        self.SWALLOW_SPEED = render.SWALLOW_SPEED
        self.SWALLOW_DECAY = render.SWALLOW_DECAY
        self.SWALLOW_LENGTH_SCALE = render.SWALLOW_LENGTH_SCALE
        self.SWALLOW_SIZE_W_SCALE = render.SWALLOW_SIZE_W_SCALE
        self.SWALLOW_SIZE_H_SCALE = render.SWALLOW_SIZE_H_SCALE
        self.SMOOTH_LERP = render.SMOOTH_LERP
        self.WIGGLE_AMPLITUDE = render.WIGGLE_AMPLITUDE
        self.WIGGLE_FREQ = render.WIGGLE_FREQ
        self.BODY_ALPHA = render.BODY_ALPHA
        self.EYE_RADIUS_SCALE = render.EYE_RADIUS_SCALE
        self.PUPIL_SCALE_BASE = render.PUPIL_SCALE_BASE
        self.PUPIL_MIN = render.PUPIL_MIN
        self.PUPIL_MAX = render.PUPIL_MAX
        self.EYE_FORWARD_SCALE = render.EYE_FORWARD_SCALE
        self.EYE_SEP_SCALE = render.EYE_SEP_SCALE
        self.SOFT_BODY_NODES = render.SOFT_BODY_NODES
        self.SOFT_BODY_SPRING_K = render.SOFT_BODY_SPRING_K
        self.SOFT_BODY_DAMPING = render.SOFT_BODY_DAMPING
        self.SOFT_BODY_HEAD_COMPRESS = render.SOFT_BODY_HEAD_COMPRESS
        self.SOFT_BODY_TAIL_ELONGATE = render.SOFT_BODY_TAIL_ELONGATE
        self.SOFT_BODY_WOBBLE_BASE = render.SOFT_BODY_WOBBLE_BASE
        self.SOFT_BODY_WOBBLE_ANG = render.SOFT_BODY_WOBBLE_ANG
        self.FONT_SIZE = render.FONT_SIZE
        self.PANEL_WIDTH = render.PANEL_WIDTH
        self.PANEL_MARGIN = render.PANEL_MARGIN
        self.PANEL_LINE_H = render.PANEL_LINE_H
        self.PANEL_ALPHA = render.PANEL_ALPHA
        self.CAMERA_ZOOM_SELECTED = render.CAMERA_ZOOM_SELECTED
        self.CAMERA_LERP = render.CAMERA_LERP
...
class PygameRenderer:
    def __init__(self, use_parallel: bool = True):
        ...
        if use_parallel:
            try:
                from parallel.quadtree import QuadTree
                self.quadtree = QuadTree(config.WINDOW_WIDTH, config.WINDOW_HEIGHT)
            except ImportError:
                logger.warning("无法导入QuadTree，使用线性查找")
        ...
        # 性能优化: 创建可复用的world layer，避免每帧分配
        W, H = config.WINDOW_WIDTH, config.WINDOW_HEIGHT
        self._world_layer = pygame.Surface((W, H), pygame.SRCALPHA)
        try:
            self._world_layer = self._world_layer.convert_alpha()
        except Exception:
            pass  # 如果convert_alpha失败，使用原surface
        ...
        self._ghost_frames: Optional[list[Tuple[float, float, float]]] = (
            None  # [(x, y, angle), ...]
        )
    ...
    def update_frame(self, world: Optional[WorldState]) -> None:
        ...
        if world is None:
            ...
            if self._stale_world_frames == self._stale_world_limit:
                logger.warning(
                    "前端连续%dx未获取到有效世界状态", self._stale_world_limit
                )
    ...
    def _fov_params(self, e: EntityState) -> Tuple[float, float]:
        ...
        if use_entity:
            ...
            if deg is None or rng is None:
                ...
                try:
                    logger.error(
                        "实体FOV属性缺失: id=%s type=%s missing=%s，使用默认参数渲染",
                        e.id,
                        e.type,
                        ",".join(missing),
                    )
    ...
    def _draw_sensor_strip(self, e: EntityState):
        ...
        pygame.draw.rect(
            self.screen, (180, 180, 190), pygame.Rect(x0, y0, container_w, h), 1
        )
        ...
        pygame.draw.rect(
            self.screen,
            (160, 160, 170),
            pygame.Rect(lg_x - 1, lg_y - 1, legend_w + 2, legend_h + 2),
            1,
        )
    ...
    def _draw_grid(self):
        ...
        for x in range(0, config.WINDOW_WIDTH, step):
            pygame.draw.line(
                self.screen, (30, 30, 40), (x, 0), (x, config.WINDOW_HEIGHT), 1
            )
        for y in range(0, config.WINDOW_HEIGHT, step):
            pygame.draw.line(
                self.screen, (30, 30, 40), (0, y), (config.WINDOW_WIDTH, y), 1
            )
    ...
    def _draw_soft_body(
        ...
        if e.speed < min_s:
            ...
            pygame.draw.circle(
                surf,
                (*color, getattr(config, "BODY_ALPHA", 180)),
                (int(d / 2), int(d / 2)),
                int(r),
            )
        ...
        wobble_ang = (
            getattr(config, "SOFT_BODY_WOBBLE_ANG", 0.06)
            * r
            * min(1.0, abs(e.angular_velocity))
        )
        ...
        wobble_ang = (
            getattr(config, "SOFT_BODY_WOBBLE_ANG", 0.04)
            * r
            * min(1.0, abs(e.angular_velocity))
        )
        ...
        for i in range(n):
            ...
            wobble = (
                0.6
                * (wobble_base + wobble_ang)
                * math.sin(t * config.WIGGLE_FREQ + i * 0.7)
            )
            target += wobble
            # 与圆形插值，防止过度前后拉伸造成分离
        ...
        pygame.draw.polygon(
            surf, (*color, getattr(config, "BODY_ALPHA", 180)), local_pts
        )
        self.screen.blit(surf, (min_x - 4, min_y - 4))

    def _draw_swallow_band(
        self, x: float, y: float, move_a: float, r: float, prog: float, amp: float
    ):
        ...
        pygame.draw.ellipse(
            surf, (*config.SWALLOW_COLOR, alpha), pygame.Rect(3, 3, bw, bh)
        )
    ...
    def _process_events(self, world: WorldState) -> None:
        ...
        try:
            if getattr(world, "counters", None) and isinstance(world.counters, dict):
                self._stats["predations"] = int(
                    world.counters.get("predations", self._stats.get("predations", 0))
                )
                self._stats["births"] = int(
                    world.counters.get("births", self._stats.get("births", 0))
                )
        ...
        for ev in getattr(world, "events", []) or []:
            ...
            if ev_type == "predation":
                ...
                if actor_id:
                    ...
                    # 吞咽带与喂食脉冲
                    self._feed_pulse[actor_id] = min(
                        config.FEED_PULSE_MAX,
                        self._feed_pulse.get(actor_id, 0.0)
                        + gain * config.FEED_PULSE_GAIN,
                    )
                    self._swallow_prog[actor_id] = 1.0
                    self._swallow_amp[actor_id] = max(
                        self._swallow_amp.get(actor_id, 0.0), min(1.0, 0.6 + gain * 0.3)
                    )
                    self._tick_swallow[actor_id] = True
                    self._stats["predations"] = self._stats.get("predations", 0) + 1
                    # 按捕食者统计击杀次数
                    self._predation_count[actor_id] = (
                        self._predation_count.get(actor_id, 0) + 1
                    )
            elif ev_type == "breed":
                ...
                if parent_id:
                    self._offspring_fallback[parent_id] = (
                        self._offspring_fallback.get(parent_id, 0) + 1
                    )
                    if cid:
                        parent = next(
                            (e for e in world.entities if e.id == parent_id), None
                        )
                        if parent is not None:
                            self._gen_fallback[cid] = max(
                                self._gen_fallback.get(cid, 0),
                                int(parent.generation) + 1,
                            )

    def _draw_entity(
        self, e: EntityState, gaze_dir: Tuple[float, float], gaze_dist: float
    ):
        ...
        if self.selected_id == e.id and self._fov_mode != "off":
            if self._fov_mode == "wedge":
                ...
                p1 = (x, y)
                p2 = (
                    x + math.cos(a - half) * fov_range,
                    y + math.sin(a - half) * fov_range,
                )
                p3 = (
                    x + math.cos(a + half) * fov_range,
                    y + math.sin(a + half) * fov_range,
        ...
        for cx, cy in (left_eye, right_eye):
            pygame.draw.circle(
                self.screen, config.EYE_WHITE, (int(cx), int(cy)), int(eye_r)
            )
            # 仅在视野(FOV)范围内偏移瞳孔方向
            gx, gy = gaze_dir
            ga = math.atan2(gy, gx)
            ...
            pupil_sz = int(
                ...
                * max(
                    config.PUPIL_MIN,
                    min(config.PUPIL_MAX, config.PUPIL_SCALE_BASE * pupil),
                )
            )
        ...
        if e.type == "hunter":
            ...
            if (
                (not self._tick_swallow.get(e.id))
                and delta_e > config.FEED_ENERGY_DELTA_THRESHOLD
                and not is_spawnling
            ):
                pulse_amp = min(
                    config.FEED_PULSE_MAX, pulse_amp + delta_e * config.FEED_PULSE_GAIN
                )
                swallow_p = 1.0
            ...
            if pulse_amp > 0.0:
                pulse_amp = max(
                    0.0,
                    pulse_amp - config.FEED_PULSE_DECAY * max(0.0, self._last_dt_sec),
                )
                scale = (
                    config.FEED_RING_MIN_SCALE
                    + (config.FEED_RING_MAX_SCALE - config.FEED_RING_MIN_SCALE)
                    * pulse_amp
                )
                ring_r = int(r * scale)
                ...
                pygame.draw.circle(
                    ring,
                    (*config.FEED_FLASH, alpha),
                    (ring_r + 4, ring_r + 4),
                    ring_r,
                    config.FEED_RING_THICKNESS,
                )
            ...
            if swallow_p > 0.0 and swallow_amp > 0.02:
                ...
                swallow_p = max(
                    0.0, swallow_p - config.SWALLOW_SPEED * max(0.0, self._last_dt_sec)
                )
                swallow_amp = max(
                    0.0,
                    swallow_amp - config.SWALLOW_DECAY * max(0.0, self._last_dt_sec),
                )
        ...
        if self.selected_id == e.id:
            pygame.draw.circle(
                self.screen, (255, 255, 255), (int(x), int(y)), int(r + 3), 1
            )
        # 更新前一帧位置
    ...
    def _draw_debug_panel(self, world: WorldState):
        if not self.show_debug:
            return
        panel_h = config.PANEL_MARGIN * 2 + config.PANEL_LINE_H * 16  # 增加行数
        ...
        def write(line: int, text: str):
            ...
            self.screen.blit(
                img,
                (config.PANEL_MARGIN, config.PANEL_MARGIN + line * config.PANEL_LINE_H),
            )
        ...
        write(0, f"Tick: {world.tick} | Total: {len(world.entities)}")
        write(1, f"Hunters: {len(hunters)} | Preys: {len(preys)}")
        write(2, f"Paused: {self.paused} | Rays: {self.show_rays}")

        # 能量统计
        if hunters:
            avg_h_energy = sum(h.energy for h in hunters) / len(hunters)
            write(3, f"Hunter E: {avg_h_energy:.1f} avg")
        else:
            write(3, f"Hunter E: N/A")

        if preys:
            avg_p_energy = sum(p.energy for p in preys) / len(preys)
            write(
                4,
                f"Prey E: {avg_p_energy:.1f} avg (+{config.ENERGY_PRODUCTION_PREY:.1f}/s)",
            )
        else:
            write(4, f"Prey E: N/A")

        write(
            5,
            f"FOV scale: {self._fov_range_scale:.2f} | Ray Δ: {self._ray_count_delta}",
        )
        write(6, "Body: soft")

        sel = next((e for e in world.entities if e.id == self.selected_id), None)
        if sel:
            write(7, f"Selected: {sel.id[:12]}... ({sel.type})")
            write(8, f"Energy: {sel.energy:.1f} / max {sel.split_energy:.1f}")
            write(9, f"Speed: {sel.speed:.1f} | AngVel: {sel.angular_velocity:.2f}")

            gen = max(
                int(sel.generation),
                int(self._gen_fallback.get(sel.id, int(sel.generation))),
            )
            children = max(
                int(sel.offspring_count),
                int(self._offspring_fallback.get(sel.id, int(sel.offspring_count))),
            )
            write(10, f"Age: {self._fmt_age(sel.age)} | Gen: {gen}")
            write(11, f"Children: {children}")

            deg, rng = self._fov_params(sel)
            write(12, f"FOV: {deg:.1f}° / {rng:.1f}px")

            if sel.type == "hunter":
                kills = self._predation_count.get(sel.id, 0)
                write(13, f"Kills: {kills} | Digest: {sel.digestion:.2f}s")
            else:
                # 显示猎物的净能量产出
                import math

                metabolism = config.ENERGY_BASE_METABOLISM_PREY
                movement = (
                    config.ENERGY_MOVEMENT_COST
                    * (sel.speed / config.ENERGY_SPEED_REFERENCE) ** 2
                )
                turning = (
                    config.ENERGY_TURNING_COST * abs(sel.angular_velocity) / math.pi
                )
                net_production = (
                    config.ENERGY_PRODUCTION_PREY - metabolism - movement - turning
                )
                write(13, f"Net E: {net_production:+.3f}/s")
    ...
    def _pick_entity(self, world: WorldState, pos):
        ...
        for e in world.entities:
            ...
            scale = config.SPAWN_MIN_SCALE + (1.0 - config.SPAWN_MIN_SCALE) * float(
                getattr(e, "spawn_progress", 1.0)
            )
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_p:
                    ...
                    # 前端化改调：加大调整力度并平滑限制
                    self._fov_range_scale = clamp(
                        self._fov_range_scale - 0.15, 0.4, 2.4
                    )
                ...
                # 移除主体切换键（仅软体）
                    ...
                    if self._ghost_frames:
                        ...
                        self._ghost_idx = min(
                            self._ghost_idx + 1, len(self._ghost_frames) - 1
                        )
    ...
    def draw_world(self, world: WorldState):
        ...
        W, H = config.WINDOW_WIDTH, config.WINDOW_HEIGHT
        # 性能优化: 复用预创建的world_layer，避免每帧分配1.92MB内存
        world_layer = self._world_layer
        # 清空layer (完全不透明背景)
        ...
        alive = [
            e for e in world.entities if not (e.type == "hunter" and e.energy <= 0.0)
        ]
        ...
        # 更新QuadTree（用于加速最近邻查询）
        if self.use_parallel and self.quadtree:
            self.quadtree.clear()
            for e in alive:
                # QuadTree.insert(entity_id, x, y, radius)
                radius = getattr(e, 'radius', config.DEFAULT_RADIUS)
                self.quadtree.insert(e.id, e.x, e.y, radius)
        ...
        for e in alive:
            if e.id in self._spawn_override:
                self._spawn_override[e.id] = min(
                    1.0, self._spawn_override[e.id] + config.SPAWN_GROW_RATE * dt
                )
        ...
        for e in alive:
            fov_deg, fov_range = self._fov_params(e)
            # 默认：沿朝向看向前方
            best_vec: Tuple[float, float, float] = (
                math.cos(e.angle),
                math.sin(e.angle),
                fov_range,
            )
            ...
            if hit:
                ...
                # 2) 回退：最近实体（欧氏距离）- 使用QuadTree加速
                best_tid: Optional[str] = None

                if self.use_parallel and self.quadtree:
                    # 使用QuadTree加速查询（O(log n)）
                    search_radius = fov_range * 1.5  # 稍微扩大搜索范围
                    nearby_ids = self.quadtree.query_circle(e.x, e.y, search_radius)
                    ...
                    for other_id in nearby_ids:
                        if other_id == e.id:
                            continue
                        # 从alive中找到对应实体
                        other = next((o for o in alive if o.id == other_id), None)
                        if other is None:
                            continue
                        dx = other.x - e.x
                        dy = other.y - e.y
        ...
        # 实体（绘制到离屏层）
        for e in alive:
            gaze = nearest_map.get(
                e.id, (math.cos(e.angle), math.sin(e.angle), e.fov_range)
            )
        ...
        # 性能优化: 使用快速scale代替smoothscale (速度提升10倍，质量略降)
        ...
        if self._ghost_frames:
            if self._ghost_play:
                ...
                if step > 0:
                    self._ghost_idx = min(
                        self._ghost_idx + step, len(self._ghost_frames) - 1
                    )
            ...
            ge = SimpleNamespace(
                id="ghost", speed=gspeed, angular_velocity=0.0, radius=g_r
            )
...
def load_ghost_frames(
    renderer: PygameRenderer, frames: list[Tuple[float, float, float]]
):
